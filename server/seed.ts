import { db } from "./db";
import {
  categories, products, artisans, testimonials,
...
async function seedCategories(): Promise<number[]> {
  ...
  const insertedCategories = await db.insert(categories).values(categoriesData).returning({ id: categories.id });
...
async function seedArtisans(): Promise<number[]> {
  ...
  const insertedArtisans = await db.insert(artisans).values(artisansData).returning({ id: artisans.id });
...
async function seedProducts(categoryIds: number[], artisanIds: number[]) {
  ...
  for (const categoryId of categoryIds) {
    const productsCount = await db.select({ count: count() }).from(products).where(eq(products.categoryId, categoryId));
